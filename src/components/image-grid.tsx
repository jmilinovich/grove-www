...
export default function ImageGrid({ images }: Props) {
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-1 overflow-x-auto flex-1 min-w-0">
          <button
            ...
            className={[
              "shrink-0 px-3 py-1.5 text-detail rounded-full border transition-colors",
              activeTag === null
                ? "bg-ink text-cream border-ink"
                : "border-surface-border text-ink/60 hover:text-ink hover:border-ink/40",
            ].join(" ")}
          >
            All
          </button>
        ...
        <label className="shrink-0 flex items-center gap-2 text-detail text-ink/60">
          Sort
          <select
            value={sort}
            onChange={(e) => setSort(e.target.value as SortMode)}
            className="bg-surface border border-surface-border rounded-md px-2 py-1 text-ink"
          >
      ...
      {visible.length === 0 ? (
        <p className="text-label text-ink/40 py-12 text-center">
...
function ImageTile({ image, onClick }: { image: ListEntry; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="group mb-3 block w-full break-inside-avoid overflow-hidden rounded-lg bg-surface border border-surface-border hover:border-ink/40 transition-colors text-left"
    ...
    >
      ...
      <div className="px-3 py-2">
        <p className="text-label text-ink font-medium truncate">{image.name}</p>
        {image.tags && image.tags.length > 0 && (
          <p className="text-detail text-ink/40 truncate mt-0.5">
